| `Network_List_All -> "Annuaire"
| `Network_List_Filter -> "Filtrer par étiquette"
| `Network_List_Empty -> "Aucune communauté trouvée"
| `Network_Title -> "Le Réseau des communautés - RunOrg"
| `Network_Search -> "Chercher"

| `Network_Missing -> "Votre communauté n'apparaît pas dans la liste ?"
| `Network_Missing_Create -> "Créez gratuitement son espace privé"

| `Network_News_Title -> "Actualités - Le Réseau des communautés - RunOrg"
| `Network_News_Explain -> "Cette page recense les actualités des communautés du réseau RunOrg. Vous pouvez vous abonner individuellement aux communautés dont les actualités vous intéressent."
