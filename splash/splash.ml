...
(*  page "/collectivites/pricing" "RunOrg - Tarifs Collectivités"
  ...
  (* END FOOTER ------------------------------------------------------------- *)
] and header id ~title ~text ?trynow menu = 
  id, fun subsection -> 
    ( call "VSplashBlocks.PageHead.render" 
	[ obj [ "title", string title ;
		"text",  string text 
	      ] ] 	
    ) :: begin if (trynow, menu) = (None, []) then [] else 
    
    [ call "VSplashBlocks.Submenu.render"
...
and page url title ?section ?head ?subsection layers = 

  let render = String.concat " |- " 
      (List.map (fun f -> f ^ " _i18n")
	 ( [page_title section] 
  ...
  "let _ = " ^
  (call "CCore.profileSessionRegister" 
     [ call "new O.Action.controller" [ "`Core" ; string url ] ;
       "(fun _ _ r -> " 
       ^ "Run.eval (new CouchDB.init_ctx) ("
       ^ call "CCore.render"
	   [ 
	     "~theme:(" ^ string "splash" ^ ",`RunOrg)" ;
	     "~title:(return $ View.esc " ^ string title ^ ")" ;
	     "~body:(return (" ^ render ^ "))" ;
	     "r"
	   ] 
       ^ "))"
...
and bool = function true -> "true" | false -> "false"

and page_title section = 
  let sections = sections () in
    ...
    call "VSplashBlocks.Title.render" 
      [ obj [ "home", string "/" ;
	      "products", list (List.map show_section sections) 
	    ] ] 

and page_head ?head ?subsection () =
  match head with None -> [] | Some id -> 
    try 
      let header = List.assoc id (headers ()) in
      header subsection 
    with Not_found -> failwith ("Header #"^id^" does not exist")

and page_foot () = 
  let links = footer () in
  call "VSplashBlocks.Footer.render" 
...
and hr () = "(fun _ -> View.str " ^ string "<hr/>" ^ ")"

and nil () = "(fun _ c -> c)"

and composite format left right = 
  let fmt = match format with `LLR -> "`LLR" | `LR -> "`LR" | `LRR -> "`LRR" in
  call "VSplashBlocks.Composite.render" 
    [ obj [ "kind",  fmt ;
	    "left",  left ;
	    "right", right ] ]

and video ~height ~poster sources = 
  call "VSplashBlocks.Video.render" 
...
and youtube url = 
  call "VSplashBlocks.Youtube.render"
    [ string url ] 

and bullets ~title ~subtitle ?(ordered=true) items = 
  call "VSplashBlocks.Bullet.render"
    [ obj [ "title", string title ;
	    "subtitle", string subtitle ;
	    "ordered", bool ordered ; 
	    "items", list (List.map string items) ]]

and pride ~title ?subtitle ?link text = 
  call "VSplashBlocks.Pride.render"
    [ obj [ "title", string title ;
	    ...
	    | None -> "None"
	    | Some (text,link) -> 
		"Some (" ^ string text ^ "," ^ string link ^")") 
	  ]]

and price title subtitle text = 
  call "VSplashBlocks.Price.render"
    [ obj [ "title", string title ;
	    "subtitle", string subtitle ;
	    "text", string text ]]

and image ?copyright url = 
  call "VSplashBlocks.Image.render" 
    [ obj [ "url", string url ;
...
and ribbon inner = 
  call "VSplashBlocks.Ribbon.render" [ inner ]  

and facebook () = 
  call "VSplashBlocks.Facebook.render" [ "()" ] 

and important title text = 
  call "VSplashBlocks.Important.render"
    [ obj [ "title", string title ;
	    "text", string text ]]  

and recommend ~title ~subtitle items = 
  call "VSplashBlocks.Recommend.render"
    [ obj [ "title", string title ;
	    "subtitle", string subtitle ;
	    "items", list (List.map (fun (who,org,quote) -> 
	      obj [ "who", string who ;
		    "org", string org ;
		    "quote", string quote ]) items)
	  ]]

and offer ~title ~price text inc = 
  call "VSplashBlocks.Offer.render"
    [ obj [ "title", string title ;
	    "text",  string text ;
	    "inc",   list (List.map string inc) ;
	    "price", string price ] ]

and backdrop_head ~title ~image ~text ~url ~action = 
  call "VSplashBlocks.BackdropHead.render"
    [ obj [ "title",  string title ;
	    "text",   string text ;
	    "image",  string image ;
...
and pricing ~foot cols rows = 
  ...
  and rows = List.map
	...
	obj [ "label", string label ; 
	      "cells", list (List.map (function 
		| `Tick -> obj [ "ticked", bool true ; "text", "None" ; "link", "None" ]
		| `NoTick -> obj [ "ticked", bool false ; "text", "None" ; "link", "None" ]
		| `Text t -> obj [ "ticked", bool false ; "text", "Some" ^ string t ; "link", "None" ]
		| `Link (l,t) -> obj [ "ticked", bool false ; "text", "Some " ^ string t ; "link", "Some " ^ string l]
				      ) cells) ]
      ) rows
  in
  
  call "VSplashBlocks.Pricing.render"
    [ obj [ "cols", list cols ; "rows", list rows ; "foot", string foot ]] 
      
and included () = 
  "let header = " ^ page_title None ^ " and footer = " ^ page_foot () 

let _ = 
  try 
    let code = 
      "open Ohm open Ohm.Universal open BatPervasives " 
      ^ "let _i18n = MModel.I18n.load (Id.of_string " 
      ^ string "i18n-common-fr"
      ^ ") `Fr "
      ^ code () 
      ^ included () 
    in
    let path = "../ocaml/gen/gSplash.ml" in
