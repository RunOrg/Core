...
module SolveArgs = Fmt.Make(struct type json t = (INotif.Plugin.t * INotif.Solve.t) end)
...
let solve key =
  ...
  let! nids = ohm (SolvableView.doc_query ~startkey:key ~endkey:key ~endinclusive:true ~limit:5 ()) in
...
module Register = functor(P:PLUGIN) -> struct
  ...
  let () = add_check (fun () -> if !render = None then failwith 
      (Printf.sprintf "Notif plugin %s : no renderer defined" (INotif.Plugin.to_string P.id)))
