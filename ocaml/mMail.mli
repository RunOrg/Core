...
module Spam : sig
  val get : [<`Block|`IsSelf] IUser.id -> 'a IInstance.id -> (#O.ctx,bool option) Ohm.Run.t
  val set : ?mid:IMail.t -> [<`Block|`IsSelf] IUser.id -> 'a IInstance.id -> bool -> (#O.ctx,unit) Ohm.Run.t
end
...
val solve : IMail.t -> (#O.ctx,unit) Ohm.Run.t
