...
let items self feed = 
  ...
  let! htmls = ohm $ Run.list_filter (CItem.render self) items in 
  return htmls

let feed_rw self feed wfeed = 
  let! items = ohm $ O.decay (items self feed) in 
  O.Box.fill (Asset_Wall_Feed.render (object
    method items = items
  end))

let feed_ro self feed = 
  let! items = ohm $ O.decay (items self feed) in 
...
let box self feed =
  match feed with 
    ...
    | Some feed -> let! writable = ohm (O.decay (MFeed.Can.write feed)) in
		   match writable with 
		     | None       -> feed_ro self feed
		     | Some wfeed -> feed_rw self feed wfeed
